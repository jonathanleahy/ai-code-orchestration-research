...
def extract_file_blocks(content, workdir):
    ...
    # Lenient: allow 2-3 dashes, optional trailing dashes on END FILE (model may truncate)
    pattern = r"-{2,3}\s*FILE:\s*([\w./\-]+)\s*-{2,3}\n(.*?)\n-{2,3}\s*END FILE\s*-{0,3}"
    matches = re.findall(pattern, content, re.DOTALL)

    # If regex missed the last block (truncated END marker), try splitting on FILE markers
    if len(matches) < content.count("FILE:") - content.count("END FILE"):
        parts = re.split(r'-{2,3}\s*FILE:\s*', content)
        for part in parts[1:]:  # skip content before first FILE
            header_end = part.find("---")
            if header_end > 0:
                filepath = part[:header_end].strip()
                rest = part[header_end+3:].strip()
                end_marker = re.search(r'\n-{2,3}\s*END FILE', rest)
                body = rest[:end_marker.start()] if end_marker else rest
                if filepath and body and len(body) > 10:
                    matches.append((filepath, body))
    ...
    # Format 4: Raw Go code — split on package declarations or ```go blocks
    if not created and ('package ' in content or '```go' in content):
        ...
        for block in go_blocks:
            ...
            if not block or len(block) < 20:
                continue
            if 'func Test' in block and 'testing' in block:
                filepath = "model/task_test.go"
            elif 'type Task struct' in block or 'type Store struct' in block:
                filepath = "model/task.go"
            elif 'func main()' in block or 'net/http' in block:
                filepath = "main.go"
            else:
                filepath = f"output_{len(created)}.go"
            full_path = os.path.join(workdir, filepath)
            os.makedirs(os.path.dirname(full_path), exist_ok=True)
            with open(full_path, "w") as f:
                f.write(block)
